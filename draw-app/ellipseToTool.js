// Ellipse tool: based on free-hand drawing tool
// A tool for drawing ellipses to the screen. Allows the user to preview the ellipse to the current mouse position before drawing the ellipse to the pixel array. This tool was originally based on the free hand tool. We edited it so that instead of the line function, we would be drawing a circular shape. We used ellipseMode(CORNERS) in order to get the circle to be made from the centre. This made it look more professional as well as more convenient for the user. At the bottom, we understood and edited code from the mirror tool. This allows the user to have the option of no fill. However, we were unable to make it toggle.
	...
	this.draw = function(){
		...
		if(mouseIsPressed){
			...
			if(startMouseX == -1){
				...
				//update the screen with the saved pixels to hide any previous ellipse between mouse pressed and released
				updatePixels();
                // sets the stroke weight
                strokeWeight(1);
                // draws the ellipse
			...
			// save the pixels with the most recent ellipse and reset the drawing bool and start locations
			loadPixels();
